...
class PopupController extends Controller
{
    public function getPopup(Request $request)
    {
        ...
        if (isset($request->idIstana) && $request->idIstana !== null) {
            $istanaWhere = " AND tpi.i_id_istana = :idIstana ";
            $parameters['idIstana'] = $request->idIstana;
        }
        ...
        $sql = "SELECT 
                    ...
                    tpi.n_link,
                    tpi.c_gambar
                FROM tm_popup_img tpi 
                WHERE tpi.d_expired_time > NOW()
                AND tpi.c_is_publish = 1
                $istanaWhere ";

        $hasil = DB::select($sql, $parameters);

        foreach ($hasil as $value) {
            $value->c_gambar = $this->getpresignedurl(null, $value->c_gambar)->getData()->presignedUrl;
        }
